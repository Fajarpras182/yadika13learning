...
@section('title', 'Manajemen Bank Soal - Admin')
...
<div class="alert alert-primary border-0 d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    ...
    <div class="d-flex flex-wrap gap-2">
        <a href="{{ route('admin.ujian.show', $bankCtxUjian) }}" class="btn btn-sm btn-outline-dark rounded-0">Detail Ujian</a>
        <a href="{{ route('admin.bank-soal') }}" class="btn btn-sm btn-outline-secondary rounded-0">Keluar mode ujian</a>
    </div>
</div>
@endif

<div class="d-flex flex-wrap align-items-center gap-2 bank-soal-toolbar mb-3">
    <a href="{{ route('admin.bank-soal.create', $bankCtxQuery) }}" class="btn btn-tambah rounded-0">
        <i class="fas fa-plus-circle me-2"></i>Tambah
    </a>
    <button type="button" class="btn btn-success btn-import rounded-0" data-bs-toggle="modal" data-bs-target="#uploadModalAdmin">
...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-body py-3">
        <form method="GET" class="row g-3 align-items-end">
            <div class="col-md-4">
                <label for="filter_course" class="form-label small text-muted mb-1">Mata Pelajaran</label>
                <select class="form-select form-select-sm" id="filter_course" name="course_id">
                    <option value="">Semua Mata Pelajaran</option>
                    @foreach(\App\Models\Course::orderBy('nama_mata_pelajaran')->get() as $c)
            ...
            <div class="col-md-4">
                <label for="filter_guru" class="form-label small text-muted mb-1">Guru</label>
                <select class="form-select form-select-sm" id="filter_guru" name="guru_id">
            ...
            <div class="col-md-2">
                <label for="filter_status" class="form-label small text-muted mb-1">Status</label>
            ...
            <div class="col-md-2 d-flex gap-1">
                ...
                <a href="{{ route('admin.bank-soal') }}" class="btn btn-sm btn-outline-secondary">Reset</a>
            </div>
        </form>
    </div>
</div>
        ...
        <table class="table bank-soal-table align-middle">
            ...
            <tbody>
                @foreach($questions as $index => $question)
                    @php
                        $no = ($questions->currentPage() - 1) * $questions->perPage() + $index + 1;
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="bank-soal-meta">
                                ...
                                @if($question->course?->guru)
                                    <span class="text-muted">·</span> {{ $question->course->guru->nama }}
                                @endif
                                @if($bankCtxUjian && (int) $question->course_id === (int) $bankCtxUjian->course_id)
                            ...
                            <div class="bank-soal-actions d-flex flex-wrap gap-2 align-items-center">
                                ...
                                <a href="{{ route('admin.bank-soal.edit', $question->id) }}" class="btn btn-link btn-sm text-primary p-0 text-decoration-none">
...
<div class="text-center py-5 border rounded bg-white">
    ...
    <div class="d-flex justify-content-center gap-2 flex-wrap">
        <a href="{{ route('admin.bank-soal.create', $bankCtxQuery) }}" class="btn btn-dark rounded-0">
            <i class="fas fa-plus-circle me-2"></i>Tambah
        </a>
        <button type="button" class="btn btn-success rounded-0" data-bs-toggle="modal" data-bs-target="#uploadModalAdmin">
            <i class="fas fa-file-import me-2"></i>Import
        </button>
    </div>
</div>
...
<div class="modal fade" id="uploadModalAdmin" tabindex="-1" aria-labelledby="uploadModalAdminLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content rounded-0">
            <div class="modal-header border-bottom">
                <h5 class="modal-title" id="uploadModalAdminLabel">Import soal</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form method="POST" action="{{ route('admin.bank-soal.upload') }}" enctype="multipart/form-data">
                @csrf
                @if($bankCtxUjian)
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="upload_course_id_admin" class="form-label">Mata Pelajaran <span class="text-danger">*</span></label>
                        <select class="form-select" id="upload_course_id_admin" name="course_id" required>
                            <option value="">Pilih mata pelajaran</option>
                            @foreach(\App\Models\Course::with('schoolClass')->orderBy('nama_mata_pelajaran')->get() as $course)
                    ...
                    <p class="small text-muted mb-0">{{ \App\Services\BankSoalDocumentImport::supportedExtensionsMessage() }}</p>
                    <div class="mb-3">
                        <label for="file_admin" class="form-label">File <span class="text-danger">*</span></label>
                        <input type="file" class="form-control" id="file_admin" name="file" accept=".xlsx,.xls,.csv,.docx" required>
                    </div>
                </div>
...
function confirmDelete(id) {
    if (confirm('Yakin ingin menghapus soal ini? Soal yang sudah dihapus tidak dapat dikembalikan.')) {
        ...
        form.method = 'POST';
        form.action = `/admin/bank-soal/${id}`;
...
function viewQuestion(id) {
    ...
    .then(data => {
        if (data.success) {
            ...
            let content = `
                <div class="mb-3">
                    <strong>Guru:</strong> ${question.course?.guru?.nama || '-'}
                </div>
                <div class="mb-3">
            ...
            content += `
                <div class="d-flex gap-2 mt-4">
                    <a href="/admin/bank-soal/${id}/edit" class="btn btn-primary">
