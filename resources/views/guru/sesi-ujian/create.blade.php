...
@section('title', 'Tambah Sesi Ujian - Guru')
    ...
    <div class="col-md-8">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">
                    <i class="fas fa-plus me-2"></i>Buat Sesi Ujian Baru
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('guru.sesi-ujian.store') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        ...
                        <select name="ujian_id" id="ujian_id" class="form-select @error('ujian_id') is-invalid @enderror" required>
                            ...
                            @foreach($ujians as $ujian)
                                <option value="{{ $ujian->id }}">{{ $ujian->judul }} ({{ $ujian->course->nama_mata_pelajaran ?? '-' }})</option>
                            @endforeach
                        </select>
                        @error('ujian_id')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_sesi" id="nama_sesi" class="form-control @error('nama_sesi') is-invalid @enderror" value="{{ old('nama_sesi') }}" placeholder="Contoh: Sesi Pagi Kelas 10A" required>
                        @error('nama_sesi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" name="auto_assign_all" id="auto_assign_all" class="form-check-input" value="1" {{ old('auto_assign_all', true) ? 'checked' : '' }}>
                        <label for="auto_assign_all" class="form-check-label">
                            <i class="fas fa-users me-1"></i> Auto-assign semua siswa kelas ujian (Recommended)
                        </label>
                    </div>
